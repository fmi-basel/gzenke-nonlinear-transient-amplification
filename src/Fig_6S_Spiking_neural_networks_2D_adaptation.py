...
for i in range(T):
    ...
    a = a - a/tau_a * dt + b * b_last_spike[:n_exc]
    a = a * (a > 0)
    ...
    v[:n_exc] = v[:n_exc] + ((v_rest - v[:n_exc]) + g_exc[:n_exc] * (v_exc - v[:n_exc]) + (g_inh[:n_exc] + a) * (v_inh - v[:n_exc])) / float(tau_m_e) * dt
    ...
    if (i+1)%10000==0:
        ...
        plt.xlim([0, 10000])
        plt.ylim([0, 500])
        plt.savefig('paper_figures/png/Fig_6S_Spiking_neural_networks_adaptation_' + str(int(t)) + '.png')
        sio.savemat('data/spiking_neural_network/Fig_6S_Spiking_neural_networks_adaptation_' + str(int(t)) + '.mat', mdict={'spike_mat': spike_mat})
        spike_mat = np.zeros((n_exc+n_inh, 10000))
        idx = 0
...
plt.xlim([0, 60000])
plt.ylim([0, 500])
plt.savefig('paper_figures/png/Fig_6S_Spiking_neural_networks_adaptation_total.png')