...
J = sio.loadmat('data/spiking_neural_network/Fig_6S_Spiking_neural_networks_connectivity_matrix.mat')['connectivity_matrix']
...
for i in range(T):
    ...
    ext_input = np.dot(J[:, :n_exc], b_last_spike[:n_exc])
    inh_input = np.dot(J[:, n_exc:], b_last_spike[n_exc:])
    exc_input = ext_input + g_ext
    ...
    if (i+1)%10000==0:
        ...
        for m in range(n_exc+n_inh):
            spike_bins = np.where(spike_mat[m, :] != 0)[0]
            if m < n_exc:
                plt.plot(spike_bins, np.ones(len(spike_bins))*m, 'bo', markersize=5)    # excitatory neurons
            else:
                plt.plot(spike_bins, np.ones(len(spike_bins))*m, 'ro', markersize=5)
        ...
        plt.xlim([0, 10000])
        plt.ylim([0, 500])
        plt.savefig('paper_figures/png/Fig_6S_Spiking_neural_networks_' + str(int(t)) + '.png')
        sio.savemat('data/spiking_neural_network/Fig_6S_Spiking_neural_networks_' + str(int(t)) + '.mat', mdict={'spike_mat': spike_mat})
        spike_mat = np.zeros((n_exc+n_inh, 10000))
        idx = 0
...
plt.xlim([0, 60000])
plt.ylim([0, 500])
plt.savefig('paper_figures/png/Fig_6S_Spiking_neural_networks_total.png')