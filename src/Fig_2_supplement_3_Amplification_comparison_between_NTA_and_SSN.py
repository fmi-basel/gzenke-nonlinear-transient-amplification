...
for g_e_idx in range(len(l_g_e_temp)):
    ...
    l_r_e_max_NTA.append(np.max(l_r_e[50000:70000])/g_e_temp)
...
for g_e_idx in range(len(l_g_e_temp)):
    ...
    l_r_e_max_SSN.append(np.max(l_r_e[50000:70000])/g_e_temp)
...
plt.plot(l_r_e_max_NTA, linewidth=plot_line_width)
plt.plot(l_r_e_max_SSN, linewidth=plot_line_width)
...
plt.legend(['NTA', 'SSN'], prop={"family": "Arial", 'size': font_size_1}, loc='upper right')
plt.savefig('paper_figures/png/Revision_Fig_Point_2_4_NTA_SSN_amplification_index.png')
plt.savefig('paper_figures/pdf/Revision_Fig_Point_2_4_NTA_SSN_amplification_index.pdf')