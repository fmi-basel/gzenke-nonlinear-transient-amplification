...
for i in range(T):
    ...
    # E-I short-term plasticty
    x = x + beta * (U_max - x) * b_last_spike[:n_exc]
    x = x * (x > 0)
    x[x > U_max] = U_max
    x = x + (1 - x) / tau_x * dt
    x = x * (x > 0)
    ...
    if (i+1)%10000==0:
        ...
        plt.xlim([0, 10000])
        plt.ylim([0, 500])
        plt.savefig('paper_figures/png/Fig_6S_Spiking_neural_networks_EI_STP_' + str(int(t)) + '.png')
        sio.savemat('data/spiking_neural_network/Fig_6S_Spiking_neural_networks_EI_STP_' + str(int(t)) + '.mat', mdict={'spike_mat': spike_mat})
        spike_mat = np.zeros((n_exc+n_inh, 10000))
        idx = 0
...
plt.xlim([0, 60000])
plt.ylim([0, 500])
plt.savefig('paper_figures/png/Fig_6S_Spiking_neural_networks_EI_STP_total.png')